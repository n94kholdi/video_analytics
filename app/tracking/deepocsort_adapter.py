"""Deep OC-SORT adapter: official algorithm → shared TrackObservation outputs."""
...
from app.tracking.third_party.deepocsort import DeepOCSort, DeepOCSortConfig
from app.tracking.trajectories import TrajectoryBook


class DeepOCSortAdapter(BaseTracker):
    """Timestamp-aware Deep OC-SORT wrapper with normalized observations."""
    ...
    def __init__(
        ...
        *,
        ...
        _ = (reid_max_age_frames, reid_interval, reid_low_threshold, bbd_threshold, use_visual_tracking)
        ...
        lookback = float(delta_t_seconds) if delta_t_seconds is not None else max(1.0 / frame_rate, 1e-6)
        ...
        self._backend = DeepOCSort(
            DeepOCSortConfig(
                activation_threshold=activation_threshold,
                iou_threshold=float(match_threshold if iou_threshold is None else iou_threshold),
                max_age_seconds=age_seconds,
                confirmation_hits=max(1, confirmation_frames),
                delta_t_seconds=lookback,
                inertia=inertia,
                w_association_emb=w_association_emb,
                alpha_fixed_emb=alpha_fixed_emb,
                aw_param=aw_param,
                use_cmc=use_cmc,
                reid_recovery_threshold=reid_similarity_threshold,
            )
        )
