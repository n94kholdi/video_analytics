"""BoT-SORT adapter: official algorithm → shared TrackObservation outputs."""
...
from app.tracking.third_party.botsort import BoTSort, BoTSortConfig
from app.tracking.trajectories import TrajectoryBook


class BoTSortAdapter(BaseTracker):
    """Timestamp-aware BoT-SORT wrapper with normalized observations."""
    ...
    def __init__(
        ...
        *,
        ...
        iou_floor = float(match_threshold if iou_threshold is None else iou_threshold)
        birth_threshold = float(activation_threshold if new_track_threshold is None else new_track_threshold)
        ...
        self._backend = BoTSort(
            BoTSortConfig(
                ...
                new_track_threshold=birth_threshold,
                iou_threshold=iou_floor,
                max_age_seconds=age_seconds,
                confirmation_hits=max(1, confirmation_frames),
                proximity_thresh=proximity_thresh,
                appearance_thresh=max((1.0 - reid_similarity_threshold) / 2.0, 1e-6),
